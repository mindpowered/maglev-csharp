// Generated by Haxe 4.1.1

#pragma warning disable 109, 114, 219, 429, 168, 162
namespace haxe.iterators {
	public class StringIterator : global::haxe.lang.HxObject {
		
		public StringIterator(global::haxe.lang.EmptyObject empty) {
		}
		
		
		public StringIterator(string s) {
			#line 38 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
			global::haxe.iterators.StringIterator.__hx_ctor_haxe_iterators_StringIterator(this, s);
		}
		#line default
		
		protected static void __hx_ctor_haxe_iterators_StringIterator(global::haxe.iterators.StringIterator __hx_this, string s) {
			#line 32 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
			__hx_this.offset = 0;
			#line 39 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
			{
				#line 39 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
				__hx_this.s = s;
			}
			
		}
		#line default
		
		public int offset;
		
		public string s;
		
		public bool hasNext() {
			#line 46 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
			return ( this.offset < this.s.Length );
		}
		#line default
		
		public int next() {
			unchecked {
				#line 53 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
				string s = this.s;
				#line 53 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
				int index = this.offset++;
				#line 53 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
				if (( ((uint) (index) ) < s.Length )) {
					#line 53 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
					return ((int) (s[index]) );
				}
				else {
					#line 53 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
					return -1;
				}
				
			}
			#line default
		}
		
		
		public override double __hx_setField_f(string field, int hash, double @value, bool handleProperties) {
			unchecked {
				#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
				switch (hash) {
					case 1614780307:
					{
						#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
						this.offset = ((int) (@value) );
						#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
						return @value;
					}
					
					
					default:
					{
						#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
						return base.__hx_setField_f(field, hash, @value, handleProperties);
					}
					
				}
				
			}
			#line default
		}
		
		
		public override object __hx_setField(string field, int hash, object @value, bool handleProperties) {
			unchecked {
				#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
				switch (hash) {
					case 115:
					{
						#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
						this.s = global::haxe.lang.Runtime.toString(@value);
						#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
						return @value;
					}
					
					
					case 1614780307:
					{
						#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
						this.offset = ((int) (global::haxe.lang.Runtime.toInt(@value)) );
						#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
						return @value;
					}
					
					
					default:
					{
						#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
						return base.__hx_setField(field, hash, @value, handleProperties);
					}
					
				}
				
			}
			#line default
		}
		
		
		public override object __hx_getField(string field, int hash, bool throwErrors, bool isCheck, bool handleProperties) {
			unchecked {
				#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
				switch (hash) {
					case 1224901875:
					{
						#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
						return ((global::haxe.lang.Function) (new global::haxe.lang.Closure(this, "next", 1224901875)) );
					}
					
					
					case 407283053:
					{
						#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
						return ((global::haxe.lang.Function) (new global::haxe.lang.Closure(this, "hasNext", 407283053)) );
					}
					
					
					case 115:
					{
						#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
						return this.s;
					}
					
					
					case 1614780307:
					{
						#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
						return this.offset;
					}
					
					
					default:
					{
						#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
						return base.__hx_getField(field, hash, throwErrors, isCheck, handleProperties);
					}
					
				}
				
			}
			#line default
		}
		
		
		public override double __hx_getField_f(string field, int hash, bool throwErrors, bool handleProperties) {
			unchecked {
				#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
				switch (hash) {
					case 1614780307:
					{
						#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
						return ((double) (this.offset) );
					}
					
					
					default:
					{
						#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
						return base.__hx_getField_f(field, hash, throwErrors, handleProperties);
					}
					
				}
				
			}
			#line default
		}
		
		
		public override object __hx_invokeField(string field, int hash, object[] dynargs) {
			unchecked {
				#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
				switch (hash) {
					case 1224901875:
					{
						#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
						return this.next();
					}
					
					
					case 407283053:
					{
						#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
						return this.hasNext();
					}
					
					
					default:
					{
						#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
						return base.__hx_invokeField(field, hash, dynargs);
					}
					
				}
				
			}
			#line default
		}
		
		
		public override void __hx_getFields(global::Array<string> baseArr) {
			#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
			baseArr.push("s");
			#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
			baseArr.push("offset");
			#line 31 "/opt/haxe/std/haxe/iterators/StringIterator.hx"
			base.__hx_getFields(baseArr);
		}
		#line default
		
	}
}


